        ...
        .setup(|app| {
            ...
            let cloud_wv = tauri::WebviewBuilder::new(
                ...
                if payload.event() == tauri::webview::PageLoadEvent::Finished {
                    ...
                    wv.eval(
                        "try{var ep=JSON.parse(localStorage.getItem('extensionPreferences')||'{}');ep['com.kosmos-eu.folderviews.app-new-window']={extensionPointId:'com.kosmos-eu.folderviews.app-new-window',selectedExtensionIds:['com.kosmos-eu.folderviews.app-new-window-enabled']};ep['com.kosmos-eu.folderviews.app-compact']={extensionPointId:'com.kosmos-eu.folderviews.app-compact',selectedExtensionIds:['com.kosmos-eu.folderviews.app-compact-enabled']};localStorage.setItem('extensionPreferences',JSON.stringify(ep));}catch(e){}if(location.search.indexOf('appTheme')!==-1){var s=document.createElement('style');s.textContent='#mobile-nav{display:none!important}#web-nav-sidebar{display:none!important}';document.head.appendChild(s);}setTimeout(function(){try{var d=[];d.push('search:'+location.search);d.push('theme:'+localStorage.getItem('oc_currentThemeName'));var cs=document.getElementById('oc-custom-theme-stylesheet');d.push('css:'+(cs?cs.href:'NONE'));var mn=document.querySelector('#mobile-nav');d.push('mobile-nav:'+(mn?getComputedStyle(mn).display:'MISS'));var tr=document.querySelector('.oc-tbody-tr');d.push('row:'+(tr?getComputedStyle(tr).height:'MISS'));document.title='[DBG] '+d.join(' | ');}catch(e){document.title='[DBG] err:'+e;}},6000);"
                    ).ok();
                }
